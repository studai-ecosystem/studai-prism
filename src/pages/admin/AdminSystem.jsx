      ...
      {health && (
        <div className="grid gap-4 md:grid-cols-2">
          <section className="rounded-[10px] border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
            ...
            <HealthRow label="Azure OpenAI" ok={health.azureOpenAI.configured} detail={health.azureOpenAI.deployment} />
            <HealthRow label="Speech-to-text (Whisper)" ok={health.speechToText.configured} />
            <HealthRow label="Text-to-speech (neural)" ok={health.textToSpeech.configured} />
          ...
          <section className="rounded-[10px] border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
            ...
            <dl className="font-sans text-sm text-[var(--color-ink)] space-y-1">
              ...
              <div className="flex justify-between"><dt className="text-[var(--color-ink-muted)]">Judge drift</dt><dd className={`font-mono text-[12px] ${health.azureOpenAI.driftStatus === 'anchored' ? '' : 'text-[var(--color-danger)]'}`}>{health.azureOpenAI.driftStatus}</dd></div>
      ...
      {models && (
        <section className="mt-4 rounded-[10px] border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
          ...
          <p className="font-sans text-sm text-[var(--color-ink)]">
            Judge deployment <span className="font-mono text-[12px]">{models.live.judgeDeployment || '—'}</span> ·
            anchored <span className="font-mono text-[12px]">{models.live.anchoredDeployment || '—'}</span> ·
